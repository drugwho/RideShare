...
#DICTIONARIES FOR SLAVE CONTAINERS AND THEIR PIDs
ids = dict()
master_id = dict()
...
#Deletes existing structure to prevent conflicts
...
#Sets the data for each node currently present
mdata = ''
...
#Creates a node for the first slave
create_node()
...
def add_container():
    ...
    id = new_cont.id
    apiconts = apiclient.containers()
    #Adds the new slave to the dictionary
    for apicont in apiconts:
         if apicont['Id']==id:
             add_ids(apicont)
    create_node()
...
def remove_container():
    #Finds the slave with the maximum PID
    max_pid = -1
    for key in ids:
        if ids[key]>=max_pid:
            max_pid = ids[key]
            max_cid = key
    #Finds the znode to be deleted and deletes it
...
def scale():
    ...
    while(True):
        ...
        #Computes the number of slaves required
        ...
        print()
        #Adds or removes slaves based on requirement
...
def on_res(ch, method, props, body):
    #Extracts return data and status if correlation IDs match
...
def read():
      ...
      for c in ct:
          try:
              ...
              pass
      #print('---END OF LOGS---')
      #Returns appropriate values, set in on_res()
      global ret
      global status
      ret = json.dumps(ret)
...
def write():
    ...
    for c in ct:
          try:
              ...
              pass
    #print('---END OF LOGS---')
    #Returns appropriate response, set in write_res()
    global write_ret
    global write_stat
...
def crash_slave():
    ...
    return(Response(json.dumps(ret_val),status=code,mimetype='application/json'))
